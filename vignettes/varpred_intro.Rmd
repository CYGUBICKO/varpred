...
**emmeans-based averaging with varpred (Experimental!)**

We can also average input variables in `varpred` similar to `emmeans` and `effects`. Consider `mod2` in the previous example, by setting `emmeans_centered=TRUE`, we can generate predictions similar to the ones in `em2`. 

```{r varpred-emmeans-centered}
## default varpred centering
vcenter <- varpred(mod2
	, "wt"
	, modelname="vpred center"
)

## varpred emmeans centering
emcenter <- varpred(mod2
	, "wt"
	, emmeans_centered=TRUE
	, modelname="em center"
)
